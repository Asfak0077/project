    ...
    for index, record in enumerate(corpus):
        ...
        try:
            product_id, document, metadata = (
                validate_and_prepare_record(record)
            )
...
def index_corpus(
    ...
    Behavior:
      ...
      - reset=True:
            Existing vector database is completely removed first.

    Returns a summary dictionary.
    """
    print("=" * 72)
    print("VER2 - CHROMA VECTOR STORE")
    print("=" * 72)
    ...
    if reset and persist_dir.exists():
        print(
            "Reset requested. Removing existing vector DB..."
        )
...
def test_collection(
    ...
    collection_name: str = DEFAULT_COLLECTION,
    embedding_model: str = DEFAULT_EMBEDDING_MODEL,
    query: str = "laptop for coding and programming",
    n_results: int = 3,
) -> Dict[str, Any]:
    """
...
def parse_arguments() -> argparse.Namespace:
    ...
    index_parser.add_argument(
        "--reset",
        action="store_true",
        help="Delete and rebuild the vector DB.",
    )
    ...
    test_parser.add_argument(
        "--query",
        type=str,
        default="laptop for coding and programming",
    )
