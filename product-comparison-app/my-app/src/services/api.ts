...
import { Product, UserProfile, UserPreferences } from "../app/types";
  ...
  (error) => {
    ...
    if (error.response) {
      ...
      if (status === 401) {
        ...
        message = data?.detail ? (typeof data.detail === "string" ? data.detail : "Validation error in input fields.") : "Validation error.";
      } else if (data?.detail) {
        message = typeof data.detail === "string" ? data.detail : JSON.stringify(data.detail);
      }
    } else if (error.request) {
      message = "Backend server is currently unavailable. Please verify it is running on port 8000.";
    }
