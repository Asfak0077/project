...
export default function WishlistView({
  ...
  const totalValue = wishlist.reduce((acc, p) => acc + (p.price || 0), 0);
        ...
        {wishlist.map((prod, i) => (
          ...
          >
            <div>
              ...
              <div className="text-xl font-black text-slate-900 mt-1">₹{Number(prod.price || 0).toLocaleString()}</div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-4 text-xs font-semibold text-slate-700">
                <div className="flex items-center gap-1.5 p-2 bg-slate-50 border border-slate-200 rounded-xl">
                  ...
                  <span className="truncate">{prod.cpu || "Multi-Core Processor"}</span>
                </div>
                <div className="flex items-center gap-1.5 p-2 bg-slate-50 border border-slate-200 rounded-xl">
                  <Activity className="w-3.5 h-3.5 text-blue-600" />
                  <span className="truncate">{formatRamDisplay(prod.ram)}</span>
                </div>
              </div>
            </div>
