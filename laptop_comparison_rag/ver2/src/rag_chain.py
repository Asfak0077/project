...
class ProductRecord:
    ...
    All values originate from retrieved metadata.
    """
    ...
    graphics_processor: Optional[str]
...
class RAGChain:
    """
    Deterministic RAG presentation pipeline.

    Responsibilities:
        retrieval
        ↓
        structured product conversion
        ↓
        comparison table
        ↓
        factual context
        ↓
        frontend/chatbot-ready output

    It deliberately does NOT:
        - call an LLM
        - generate recommendations from imagination
        - alter product facts
        - perform conversation management
        - access SQLite memory
    """
    ...
    def run(
        ...
        return RAGResponse(
            ...
            semantic_query=(
                retrieval.query.semantic_query
            ),

            filters=filters,

            preferences=list(
                retrieval.query.preference_terms
            ),
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run the ver2 deterministic RAG chain."
        )
    )
