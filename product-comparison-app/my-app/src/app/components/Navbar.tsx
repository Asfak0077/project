        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="p-2.5 rounded-2xl bg-slate-100 hover:bg-slate-200/70 border border-slate-200 text-slate-700 relative transition-all cursor-pointer"
              title="Notifications"
            >
              <Bell className="w-4.5 h-4.5 text-slate-600" />
              {notifications.length > 0 && (
                <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-blue-600 rounded-full ring-2 ring-white animate-pulse" />
              )}
            </button>
            ...
            <AnimatePresence>
              {showNotifications && (
                <motion.div
                  ...
                  className="absolute right-0 mt-3 w-80 bg-white rounded-3xl p-4 shadow-2xl border border-slate-200 z-50 space-y-3"
                >
                  <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
                    <span className="text-xs font-extrabold text-slate-900">Activity & Price Alerts</span>
                    <span className="text-[10px] font-black text-blue-600 uppercase bg-blue-50 px-2 py-0.5 rounded-full">Live</span>
                  </div>

                  <div className="space-y-2 max-h-64 overflow-y-auto">
                    {notifications.length === 0 ? (
                      <p className="text-xs text-slate-400 py-3 text-center">No alerts at the moment.</p>
                    ) : (
                      notifications.map((n, i) => (
                        <div key={i} className="p-2.5 bg-slate-50 border border-slate-100 rounded-xl text-xs text-slate-700 font-medium leading-relaxed flex items-start gap-2">
