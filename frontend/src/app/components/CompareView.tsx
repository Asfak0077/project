...
export default function CompareView({
  ...
  const handleExportReport = () => {
    try {
      exportComparisonToPdf(products, overallWinner, comparisonData, specRows);
      setExported(true);
  ...
  return (
    <div className="py-8 space-y-8 pb-32">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            title="Export full comparison report to PDF document"
          >
            {exported ? (
              <>
          ...
          {onLaunchBattle && products.length >= 2 && (
            <button
              onClick={onLaunchBattle}
              className="px-4 py-2 bg-gradient-to-r from-rose-600 via-red-600 to-amber-600 hover:from-rose-500 hover:to-amber-500 text-white rounded-xl text-xs font-black shadow-md shadow-rose-500/25 flex items-center gap-1.5 cursor-pointer transition-all border border-rose-400/30"
            >
              <Swords className="w-3.5 h-3.5" /> AI Battle Mode ⚡
            </button>
          )}
      ...
      {/* Top Winner Card Banner - Premium Light Gold Theme */}
      {(() => {
        ...
        return (
          <div className="bg-gradient-to-r from-amber-50/70 via-white to-amber-50/70 text-slate-900 rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 shadow-sm border-2 border-amber-300 relative overflow-hidden">
            {/* Ambient Background Glow */}
            <div
              className="absolute top-0 right-1/4 w-72 h-72 rounded-full blur-3xl pointer-events-none opacity-30"
              style={{ background: "radial-gradient(circle, rgba(245,158,11,0.25), transparent 70%)" }}
            />
            
            <div className="flex items-center gap-5 relative z-10">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-amber-400 to-yellow-300 text-white flex items-center justify-center font-black shadow-md shadow-amber-400/30 shrink-0">
                <Award className="w-8 h-8" />
              </div>
              <div>
                <div className="text-[11px] font-black uppercase tracking-wider text-amber-700 flex items-center gap-1.5">
                  <Sparkles className="w-3.5 h-3.5 text-amber-600" /> Overall Benchmark Leader
                </div>
            ...
            <div className="text-left sm:text-right shrink-0 relative z-10 pt-2 sm:pt-0 border-t sm:border-t-0 border-amber-200 w-full sm:w-auto flex sm:flex-col justify-between items-center sm:items-end">
              <div className="text-xs text-amber-800 font-extrabold uppercase tracking-wider">Top Verified Score</div>
              <div className="text-3xl font-black text-amber-600 drop-shadow-xs">{overallWinner.score} / 100</div>
            </div>
          </div>
        );
      })()}
