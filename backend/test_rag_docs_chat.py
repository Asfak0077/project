...
class TestRAGDocsChat(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if not cls.test_doc:
            ...
            RAGService.process_and_index_document(cls.db, cls.test_doc.id)
            cls.db.refresh(cls.test_doc)
    ...
    def test_02_ask_what_is_processor(self):
        ...
        data = res.json()
        self.assertIn("### PROCESSOR", data["answer"])
        self.assertIn("AMD Ryzen 7", data["answer"])
    ...
    def test_03_ask_what_is_ram(self):
        ...
        self.assertEqual(res.status_code, 200)
        data = res.json()
        self.assertIn("### RAM", data["answer"])
        self.assertIn("16GB", data["answer"])
        self.assertIn("Source:", data["answer"])

    def test_04_ask_explain_battery_performance(self):
        """4. Ask: 'Explain battery performance' -> Detailed explanation with Summary + Details + Source."""
        payload = {
        ...
        self.assertEqual(res.status_code, 200)
        data = res.json()
        self.assertIn("### Battery Performance", data["answer"])
        self.assertIn("Summary:", data["answer"])
        self.assertIn("Details:", data["answer"])
        self.assertIn("Source:", data["answer"])

    def test_05_ask_summarize_document(self):
        """5. Ask: 'Summarize document' -> Structured Document Summary Card."""
        payload = {
        ...
        self.assertEqual(res.status_code, 200)
        data = res.json()
        self.assertIn("### Document Summary", data["answer"])
        self.assertIn("Product:", data["answer"])
        self.assertIn("Key Points:", data["answer"])
